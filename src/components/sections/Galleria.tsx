    ...
    return (
        <section id="galleria" className="section py-[var(--s-6)]" aria-label="Galleria">
            <div className="wrap">
                ...
                <div className="grid3 mt-[var(--s-5)] grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-4" aria-label="Galleria immagini">
                    {data.items.map((item, idx) => (
                        <figure key={idx} className="gridItem reveal m-0 border border-[color-mix(in_oklab,var(--border)_85%,transparent)] overflow-hidden bg-[linear-gradient(180deg,var(--bg-3),var(--bg))] shadow-[0_26px_110px_var(--shadow)] aspect-[3/4] relative" style={{ borderRadius: 'var(--r-ui)' }}>
